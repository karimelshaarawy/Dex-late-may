...
pub struct Liquidity_pool<'a> {
    ...
    reserve1: f64,
    klast : f64,
   pub(crate) liquidity_providers :HashMap<&'a str,f64>,
   pub(crate) total_pool_tokens : f64

}

impl<'a> Liquidity_pool<'a> {
    pub fn new<'b>( new_key:String, tokenA:&'b str, tokenB:&'b str) ->Liquidity_pool<'b>{
        Liquidity_pool{
            key:new_key,
    ...
    pub(crate) fn mint(&mut self, address_to:&'a str, token0_num:f64, token1_num:f64) -> f64{
    ...
    pub(crate) fn burn (&mut self, address_from:&'a str, pool_tokens:f64) -> (f64, f64){
